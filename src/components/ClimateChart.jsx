...
function CustomTooltip({ active, payload, label, baseline, unit }) {
  ...
  return (
    <div className="chart-tooltip">
      <p>{label}</p>
      <span>Rainfall: {rainfallPoint?.value} {unit}</span>
    ...
    <section className="chart-card climate-chart-card">
      ...
      <div className="chart-wrap">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={climate.data} margin={{ top: 18, right: 20, left: -12, bottom: 2 }}>
            <defs>
            ...
            <YAxis domain={[yMin, yMax]} tickCount={5} axisLine={false} tickLine={false}
              tick={{ fill: "#9ca3af", fontSize: 12 }} tickFormatter={(v) => `${v}`} />
            <Tooltip content={<CustomTooltip baseline={climate.baseline} unit={climate.unit} />} cursor={false} />
            <ReferenceLine y={climate.baseline} stroke="#cbd5e1" strokeDasharray="4 5"
              label={{ value: climate.baselineLabel, position: "insideTopRight", fill: "#cbd5e1", fontSize: 11 }} />
            <Area type="monotone" dataKey="rainfall" name="Rainfall"
              stroke="#38bdf8" strokeWidth={3} fill="url(#rainfallGrad)"
              dot={false} activeDot={{ r: 5, fill: "#7dd3fc", stroke: "#0b1220", strokeWidth: 2 }}
      ...
      <div className="evidence-meta">
        <div><span>Source</span><strong>{climate.source}</strong></div>
        <div><span>Method</span><strong>{climate.method}</strong></div>
        <div><span>Limitation</span><strong>{climate.limitation}</strong></div>
      </div>
    </section>
  );
}
