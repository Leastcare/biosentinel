...
function CustomTooltip({ active, payload, label, baseline, unit }) {
  ...
  return (
    <div className="chart-tooltip">
      <p>{label}</p>
      <span>Records: {recordPoint?.value} {unit}</span>
    ...
    <section className="chart-card wildlife-chart-card">
      ...
      <div className="chart-wrap">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={wildlife.data} margin={{ top: 18, right: 20, left: -12, bottom: 2 }}>
            <defs>
              <linearGradient id="wildlifeGrad" x1="0" y1="0" x2="0" y2="1">
            ...
            <Tooltip content={<CustomTooltip baseline={wildlife.baseline} unit={wildlife.unit} />} cursor={false} />
            <ReferenceLine y={wildlife.baseline} stroke="#cbd5e1" strokeDasharray="4 5"
              label={{ value: wildlife.baselineLabel, position: "insideTopRight", fill: "#cbd5e1", fontSize: 11 }} />
            <Area type="monotone" dataKey="records" name="GBIF records"
              stroke="#34d399" strokeWidth={3} fill="url(#wildlifeGrad)"
              dot={false} activeDot={{ r: 5, fill: "#6ee7b7", stroke: "#0b1220", strokeWidth: 2 }}
      ...
      <div className="evidence-meta">
        <div><span>Source</span><strong>{wildlife.source}</strong></div>
        <div><span>Method</span><strong>{wildlife.method}</strong></div>
        <div><span>Limitation</span><strong>{wildlife.limitation}</strong></div>
      </div>
    </section>
  );
}
